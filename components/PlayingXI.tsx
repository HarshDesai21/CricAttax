...
function TeamPanel({
  ...
  const initial = (): { xi: number[]; subs: number[] } => {
    ...
    // Bug fix (live-playtest report): no saved arrangement yet used to
    // default to a blind "first 11 drafted (round order)" -- draft order has
    // no idea about squad composition, so a participant whose first 11
    // picks happened to include more than 4 overseas players got an
    // invalid default seeded for them. That first save (see the "seed the
    // default" effect below) then got rejected by the server's own
    // max-4-overseas-in-XI check, so no playing_xi rows ever got created --
    // meaning `entries` stayed permanently empty and this same invalid
    // default kept getting recomputed on every render. Any actual drag they
    // made would apply locally but ALSO get rejected server-side the same
    // way, and the very next unrelated re-render (a Realtime update from
    // anyone else's activity) would then resync `lists` right back to this
    // same broken default -- from their side, every drag looked like it
    // silently reverted, no matter what they tried.
    //
    // Fixed by building the default greedily in draft order while
    // respecting the same caps the server enforces: fill the XI in
    // drafted-order, skipping an overseas player once the cap is already
    // hit (it falls to Subs instead) and continuing to scan later picks to
    // fill the XI back up to 11. This can never produce an invalid default,
    // so the very first auto-save always succeeds.
    const xi: number[] = [];
    const subs: number[] = [];
    let overseasInXi = 0;
    for (const p of squad) {
      if (xi.length < MAX_XI_SIZE && (!p.is_overseas || overseasInXi < MAX_OVERSEAS_IN_XI)) {
        xi.push(p.id);
        if (p.is_overseas) overseasInXi++;
      } else {
        subs.push(p.id);
      }
    }
