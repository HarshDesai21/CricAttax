...
function TeamPanel({
  ...
  const panelStyle = franchise
    ? {
        background: `linear-gradient(rgba(0,0,0,0.6), rgba(0,0,0,0.6)), ${franchise.primary_color}`,
        borderColor: hexToRgba(franchise.secondary_color, 0.5),
      }
    : undefined;
  ...
  const body = (
    ...
    >
      ...
      <div
        ...
        style={franchise ? { borderColor: hexToRgba(franchise.secondary_color, 0.35) } : undefined}
      >
        {franchise?.logo_asset_path && (
          <div className="h-7 w-7 flex-shrink-0 overflow-hidden rounded ring-1 ring-black/30 sm:h-8 sm:w-8 lg:h-9 lg:w-9">
            <Image
        ...
        {editable && (
          <span className="flex-shrink-0 font-body text-[9px] uppercase tracking-wide text-gold/70 lg:text-[11px]">
            Drag to arrange
          </span>
        )}
      </div>
